...
import { skinBodyAreaOptions, skinSymptomOptions } from "@/components/tracker/constants";
...
export function SkinEntryForm(props: { readOnly?: boolean }) {
  ...
  const canSave = createMemo(() => symptoms().length > 0 && !busy());
    ...
    <section class="rounded-lg border border-border bg-surface p-4 shadow-sm sm:p-5">
      <div class="mb-4 flex items-start justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-muted">Record a daily state or a specific observation.</p>
        </div>
